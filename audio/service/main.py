...
@app.get('/audio/audio/{user_id}/{video_id}/{lang}', response_class=StreamingResponse, responses={
    200: {
        'content': {
            'audio/mpeg': {}
        },
...
async def get_audio(user_id: str, video_id: str, lang: str):
    file_path = os.path.join('./localstorage', user_id, video_id, 'audio', lang + '.mp3')
    if not os.path.isfile(file_path):
        raise HTTPException(status_code=404, detail='No audio found for the given video with the given language')
...
async def get_transcription(user_id: str, video_id: str, lang: str):
    transcription_path = os.path.join('./localstorage', user_id, video_id, 'transcription', lang + '.json')
    if not os.path.isfile(transcription_path):
        raise HTTPException(status_code=404, detail='Transcription not found for given video with given language')
